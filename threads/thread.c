...
thread_awake(int64_t ticks)
{
	...
	// MIN_alarm_time = list_entry(list_begin(&sleep_list), struct thread, elem)->time_to_wakeup;
}
...
thread_get_recent_cpu (void) {
	/* TODO: Your implementation goes here */
	enum intr_level old_level = intr_disable();
	int recent_cpu_100 = fp_to_int(mult_mixed(thread_current()->recent_cpu, 100));
	intr_set_level(old_level);
